...
3. Agent uses the code agent (e.g., `codex` CLI) to fix the issue based on description and comments.
4. Commit changes with proper descriptions and push.
...
3. The agent interprets diffs as requirement changes and implements them.
4. Commit changes with proper descriptions and push.
5. Update the PR.
...
2. The agent watches GitHub repo branches and selects any that:
...
4. The agent enters the session loop.
...
4. If neither applies, wait and repeat.
